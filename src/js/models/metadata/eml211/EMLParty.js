...
define(["jquery", "underscore", "backbone", "models/DataONEObject"], function (
   ...
   * @see
   * https://eml.ecoinformatics.org/schema/eml-party_xsd.html#ResponsibleParty
  ...
  var EMLParty = Backbone.Model.extend(
    /** @lends EMLParty.prototype */ {
       ...
       * @property {Element} [options.objectDOM] - The XML DOM for this model
       */
      ...
      /**
       * Parse the object DOM to create the model @param objectDOM the XML DOM
       * to parse @return modelJSON the resulting model attributes object
       */
      parse: function (objectDOM) {
        ...
        // Email - only set it on the JSON if it exists (we want to avoid an
        // empty string value in the array)
        if (
        ...
        // Online URL - only set it on the JSON if it exists (we want to avoid an
        // empty string value in the array)
        if ($(objectDOM).find("onlineurl, onlineUrl").length) {
          // modelJSON.onlineUrl = [$(objectDOM).find("onlineurl,
          // onlineUrl").first().text()];
        ...
        // User ID - only set it on the JSON if it exists (we want to avoid an
        // empty string value in the array)
      ...
      /**
       * Parses the given XML node and sets the appropriate model attributes
       * based on the node name and value
      ...
      /**
       * Parses a person XML node and returns an object with the givenName,
       * surName, and salutation values
       * @param {Element} personXML - The XML node to parse
       * @returns {object} An object with the givenName, surName, and salutation
       * values
       */
      parsePerson: function (personXML) {
        ...
        // Concatenate all the given names into one, for now TODO: Support
      ...
      /**
       * Parses an address XML node and returns an object with the city,
       * administrativeArea, postalCode, and country values
       * @param {Element} addressXML - The XML node to parse
       * @returns {object} An object with the city, administrativeArea,
       * postalCode, and country values
       */
      ...
      /**
       * Converts this EMLParty model into XML
       * @return {string} The EML XML as a string
       */
      serialize: function () {
      ...
      /**
       * Updates the attributes on this model based on the application user (the
       * app UserModel)
       */
      ...
      updateDOM: function () {
        ...
        // If there is already an XML node for this model and it is the wrong
        //   type, then replace the XML node contents
        if (objectDOM && objectDOM.nodeName != type.toUpperCase()) {
          objectDOM = $(document.createElement(type)).html(objectDOM.innerHTML);
        }
        // If there is already an XML node for this model and it is the correct
        //   type, then simply clone the XML node
        else if (objectDOM) {
        ...
        // There needs to be at least one individual name, organization name, or
        // position name
        if (
          this.nameIsEmpty() &&
          ...
          function (address, i) {
            ...
            _.each(address.deliveryPoint, function (deliveryPoint, ii) {
              ...
              if (!delPointNode) {
                ...
                // Add the deliveryPoint node to the address node Insert after
                // the last deliveryPoint node
          ...
          function (id) {
            ...
            if (this.isOrcid(id)) {
              ...
              if (id.indexOf("http") == -1) {
                // If this is an ORCID with just the 16-digit numbers and
                // hyphens, then add the https://orcid.org/ prefix to it
                if (id.length == 19) {
                ...
                // If it starts with "www.orcid.org", then add the "https" prefix
                // and remove the "www"
        ...
        // role If this party type is not an associated party, then remove the
        // role element
      ...
      /**
       * Adds this EMLParty model to it's parent EML211 model in the appropriate
       * role array
       *
       * @return {boolean} - Returns true if the merge was successful, false if
      ...
      /**
       * Checks if this EMLParty model is empty
       * @return {boolean} Returns true if the model is empty
       */
      isEmpty: function () {
        ...
        // Check each value in the model that gets serialized to see if there is
        // a value
        for (var i in attributes) {
          // Get the value from the model for this attribute
          var modelValue = this.get(attributes[i]);

          // If this is an array, then we want to check if there are any values
          // in it
        ...
        if (this.get("address")) {
          ...
          _.each(this.get("address"), function (address) {
            // Delivery point is an array so we need to check the first and
            // second values of that array
            if (
        ...
        // If we never found a value, then return true because this model is
        // empty
        return true;
      },

      /**
       * Returns the node in the given EML snippet that the given node type
      ...
      /**
       * Create a random number and set it as the xmlID attribute
       */
      createID: function () {
        this.set(
          "xmlID",
      ...
      /**
       * Set the type of this EMLParty model based on the roles
       */
      setType: function () {
        if (this.get("roles")) {
      ...
      /**
       * Sets the rootDataPackage.packageModel.changed attribute to true when
       * this model changes
       */
      ...
      /**
       * Remove this EMLParty model from it's parent EML211 model.
       */
      ...
      validate: function () {
        ...
        // If there are no values in this model that would be serialized, then
        // the model is valid
        if (
          !this.get("organizationName") &&
          !this.get("positionName") &&
        ...
        // The EMLParty must have either an organization name, position name, or
        // surname. It must ALSO have a type or role.
        if (
          !this.get("organizationName") &&
          !this.get("positionName") &&
          (!this.get("individualName") || !surName)
        ...
        // If there is a first name and no last name, then this is not a valid
        // individualName
        else if (
          givenName[0]?.length &&
          !surName &&
        ...
        // Check that each required field has a value. Required fields are
        // configured in the {@link AppConfig}
        let roles =
        ...
        for (role of roles) {
          ...
          requiredFields?.forEach((field) => {
            ...
            if (!currentVal || !currentVal?.length) {
              errors[field] = `Provide a${
                ["a", "e", "i", "o", "u"].includes(field.charAt(0)) ? "n " : " "
              } ${field}. `;
            }
          });
        }
      ...
      /**
       * Returns true if the given string is an ORCiD. Otherwise, returns false.
      ...
      isOrcid: function (username) {
        ...
        // The ORCID checksum algorithm to determine is a character string is an
        // ORCiD
      ...
      /**
       * Clones all the values of this array into a new JS Object. Special care
       * is needed for nested objects and arrays This is helpful when copying
       * this EMLParty to another role in the EML
       */
      copyValues: function () {
        ...
        if (modelValues.address.length) {
          _.each(modelValues.address, function (address, i) {
            ...
            // The delivery point is an array of strings, so properly clone the
            // array
      ...
      /**
       * getName - For an individual, returns the first and last name as a
       * string. Otherwise, returns the organization or position name.
       *
       * @return {string} Returns the name of the party or an empty string if
       * one cannot be found
       *
       * @since 2.15.0
       */
       ...
       * model contains only empty values. Otherwise, returns false. This is
       * just a shortcut for manually checking each name field individually.
       *
       * @return {boolean}
       */
      ...
      /**
       * @type {object[]}
       * @property {string} label - The name of the party category to display to
       * the user
       * @property {string} dataCategory - The string that is used to represent
       *  this party. This value should exactly match one of the strings listed
       *  in EMLParty typeOptions or EMLParty roleOptions.
       * @property {string} description - An optional description to display
       *  below the label to help the user with this category.
       * @property {boolean} createFromUser - If set to true, the information
       *  from the logged-in user will be used to create an EML party for this
       *  category if none exist already when the view loads.
       * @property {number} limit - If the number of parties allowed for this
       *  category is not unlimited, then limit should be set to the maximum
       *  allowable number.
       * @since 2.21.0
       */
      partyTypes: [
      ...
      /**
       * See {@link DataONEObject#formatXML}
       */
